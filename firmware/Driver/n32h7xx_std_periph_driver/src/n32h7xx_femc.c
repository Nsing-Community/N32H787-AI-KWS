...
/**
*\*\name    FEMC_SetCommand.
*\*\fun     Set command to set memory manager .
...
FlagStatus FEMC_GetFlag(uint32_t FEMC_FLAG)
{
    ...
    /* Check the status of the specified FEMC flag */
    if ((FEMC->FEMC_STS & FEMC_FLAG) != (uint32_t)RESET)
    {
        /* specified FEMC flag is set */
        bitstatus = SET;
    }
    else
    {
        /* specified FEMC flag is reset */
        bitstatus = RESET;
    }

    /* Return the specified FEMC flag status */
    return  bitstatus;
}
...
*\*\fun     Enables or disables the ADV singal same as multiplexed mode in asynchronous mode.
*\*\param   NewState: new state of the multiplexed mode.
*\*\          - ENABLE
*\*\          - DISABLE
*\*\return  none
