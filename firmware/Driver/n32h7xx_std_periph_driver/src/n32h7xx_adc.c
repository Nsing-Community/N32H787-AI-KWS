...
void ADC_ConfigRegularChannel(ADC_Module* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
    ...
    /*delay  4*ADC_CLK at least for synchronizing*/
    for(index = 0;index<80;index++)//M7CLK = 600MHz,delay = 1us
    {
        __NOP();
    }
}
...
void ADC_StartInjectedConv(ADC_Module *ADCx)
{
    ...
    /* Wait Start Bit Set */
    while((ADCx->CTRL3 & ADC_INJ_START_MASK) != ADC_INJ_START_MASK)
        ;
}
...
void ADC_StartRegularConv(ADC_Module *ADCx)
{
    ...
    /* Wait Start Bit Set */
    while((ADCx->CTRL3 & ADC_REG_START_MASK) != ADC_REG_START_MASK)
        ;
}
...
{
    RCC_ClocksTypeDef RCC_GetClocksStructure;
    uint32_t temp;
    
    RCC_GetClocksFreqValue(&RCC_GetClocksStructure);
    if(ADC_ClkMode == ADC_CTRL3_CKMOD_AHB)
    {
        ADC_SelectClockMode(ADCx, (uint32_t)ADC_CTRL3_CKMOD_AHB);
            
        temp = RCC_GetClocksStructure.AHB1ClkFreq/20000000; //ADC MAX CLK 20M
        if((RCC_GetClocksStructure.AHB1ClkFreq%20000000) >= temp)
            temp = temp + 1;
                
        if(temp >= RCC_ADCPrescaler)
        {
            RCC_ADCPrescaler = temp;
        }
    }
    else
    {
        ...
        if(Pll_CLK_source == RCC_ADCPLLCLK_SRC_PLL2B)
        {
            temp = RCC_GetClocksStructure.PLL2BClkFreq/20000000; //ADC MAX CLK 20M
            if((RCC_GetClocksStructure.PLL2BClkFreq%20000000) >= temp)
                temp = temp + 1;
        }
        else if(Pll_CLK_source == RCC_ADCPLLCLK_SRC_PLL1B)
        {
            temp = RCC_GetClocksStructure.PLL1BClkFreq/20000000;
            if((RCC_GetClocksStructure.PLL1BClkFreq%20000000) >= temp)
                temp = temp + 1;                    
        }
        else if(Pll_CLK_source == RCC_ADCPLLCLK_SRC_PLL3B)
        {
            temp = RCC_GetClocksStructure.PLL3BClkFreq/20000000; //ADC MAX CLK 20M  
            if((RCC_GetClocksStructure.PLL3BClkFreq%20000000) >= temp)
                temp = temp + 1;                    
        }
        else if(Pll_CLK_source == RCC_ADCPLLCLK_SRC_PLL3C)
        {
            temp = RCC_GetClocksStructure.PLL3CClkFreq/20000000; //ADC MAX CLK 20M  
            if((RCC_GetClocksStructure.PLL3CClkFreq%20000000) >= temp)
                temp = temp + 1;                    
        }
        else
        {
