...
*\*\        - SHRTIM_OUT_NO_IDLE or SHRTIM_OUT_IDLE_WHEN_BURST
...
/**
*\*\func  Set the output IDLE mode.
*\*\note This function must not be called when the burst mode is active
*\*\param  SHRTIMx High Resolution Timer instance
*\*\param  Output This parameter can be one of the following values:
*\*\        -SHRTIM_OUTPUT_TA1
*\*\        -SHRTIM_OUTPUT_TA2
*\*\        -SHRTIM_OUTPUT_TB1
...
*\*\        -SHRTIM_OUT_NO_IDLE
*\*\        -SHRTIM_OUT_IDLE_WHEN_BURST
*\*\return None
**/
 void SHRTIM_OUT_SetIdleMode(SHRTIM_Module *SHRTIMx, uint32_t Output, uint32_t IdleMode)
 {
   uint32_t iOutput = (uint8_t)(POSITION_VAL(Output) - POSITION_VAL(SHRTIM_OUTPUT_TA1));
   __IO uint32_t *pReg = (__IO uint32_t *)((uint32_t)((uint32_t)(&SHRTIMx->sTimerxRegs[0].TxOUT) + REG_OFFSET_TAB_TxOUT[iOutput]));
   MODIFY_REG(*pReg, (SHRTIM_TXOUT_IDLEM1 << (REG_SHIFT_TAB_TxOUT[iOutput])), (IdleMode << (REG_SHIFT_TAB_TxOUT[iOutput])));
 }
 
/**
*\*\func  Get actual output IDLE mode.
*\*\param  SHRTIMx High Resolution Timer instance
*\*\param  Output This parameter can be one of the following values:
*\*\        -SHRTIM_OUTPUT_TA1
*\*\        -SHRTIM_OUTPUT_TA2
...
*\*\        -SHRTIM_OUTPUT_TF1
*\*\        -SHRTIM_OUTPUT_TF2
*\*\return IdleMode This parameter can be one of the following values:
*\*\        -SHRTIM_OUT_NO_IDLE
*\*\        -SHRTIM_OUT_IDLE_WHEN_BURST
**/
 uint32_t SHRTIM_OUT_GetIdleMode(const SHRTIM_Module *SHRTIMx, uint32_t Output)
 {
   uint32_t iOutput = (uint8_t)(POSITION_VAL(Output) - POSITION_VAL(SHRTIM_OUTPUT_TA1));
   const __IO uint32_t *pReg = (__IO uint32_t *)((uint32_t)((uint32_t)(&SHRTIMx->sTimerxRegs[0].TxOUT) + REG_OFFSET_TAB_TxOUT[iOutput]));
   return (READ_BIT(*pReg, (uint32_t)(SHRTIM_TXOUT_IDLEM1) << REG_SHIFT_TAB_TxOUT[iOutput]) >> REG_SHIFT_TAB_TxOUT[iOutput]);
 }
