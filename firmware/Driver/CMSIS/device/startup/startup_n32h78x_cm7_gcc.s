...
/* SystemInit configured the ITCM/DTCM split. Copy every function linked in
   .QuickCodes from Flash into executable ITCM before calling application
   code, including SystemPostInit(). */
  ldr r0, =_sQuickCodes
  ldr r1, =_eQuickCodes
  ldr r2, =_siQuickCodes
  movs r3, #0
  b LoopCopyQuickCodes
...
/* PWR_ConfigSupply resides in .QuickCodes. */
  bl SystemPostInit

/* The complete model must be in DTCM before any TFLM constructor runs. */
  ldr r0, =_sperson_model
  ldr r1, =_eperson_model
  ldr r2, =_siperson_model
1:
  cmp r0, r1
  bcs 2f
  ldr r3, [r2], #4
  ...
  b 1b
2:
/* Verify the full copy, including the final words, before using the model. */
  ldr r0, =_sperson_model
  ldr r2, =_siperson_model
VerifyModelCopy:
  cmp r0, r1
  bcs ModelCopyVerified
  ldr r3, [r2], #4
  ldr r4, [r0], #4
  cmp r3, r4
  bne ModelCopyFailed
...
ModelCopyFailed:
  b ModelCopyFailed
ModelCopyVerified:
/* The arena is NOLOAD DTCM and must be clear before TFLM construction. */
  ldr r0, =_sperson_arena
  ldr r1, =_eperson_arena
  movs r2, #0
3:
  cmp r0, r1
  bcs 4f
